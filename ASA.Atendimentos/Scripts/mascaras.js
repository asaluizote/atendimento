...
function formataValor(campo, evt, max) {
    //MaxLength
    $(campo).attr('maxlength', max)
...
//Testado no Chrome, Firefox e Internet Explorer
function formataData(campo, evt) {
    ...
    vlrCM = campo.value; //Campo com a máscara
    tamCM = vlrCM.length; //tamanho com a máscara
    ...
    if (tecla == 8) {
        if (tam >= 8)
            ...
            campo.value = vlrCM.substr(0, tamCM);
    }
...
function isDate(campo) {
    var data = /^(((((0[1-9])|(1\d)|(2[0-8]))\/((0[1-9])|(1[0-2])))|((31\/((0[13578])|(1[02])))|((29|30)\/((0[1,3-9])|(1[0-2])))))\/((20[0-9][0-9])|(19[0-9][0-9])))|((29\/02\/(19|20)(([02468][048])|([13579][26]))))$/;
    var valor = campo.value;

    if (valor != null && valor != "") {
        if (!data.test(valor)) {
            alert("A Data " + valor + " não é válida.<br>Digite novamente!");
...
function verificaTelefone(campo, evt) {
    var controle = campo;
    vr = campo.value = filtraNumeros(filtraCampo(campo));
    tam = vr.length;
    if (tam > 1) {
        if (tam > 9 && tam <= 11)
            formataTelefoneSP(campo, evt);
        else {
            campo.value = "";
            alert("O telefone está no formato incorreto. \n Digite: (00) 0000?-0000");
            globalvar = campo;
            globalvar.focus()
        }
    }
...
//Permite 11 dígitos
function formataTelefoneSP(campo, evt) {
